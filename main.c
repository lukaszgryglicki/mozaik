...
void* thread_routine(void* arg)
{
    ...
    memcpy((void*)&ti, (char*)arg+sizeof(void*), sizeof(int));
    memcpy((void*)&num, (char*)arg+sizeof(void*)+sizeof(int), sizeof(int));
    memcpy((void*)&nthr, (char*)arg+sizeof(void*)+2*sizeof(int), sizeof(int));
    printf("Thread %p, %d/%d, %d\n", (void*)fnames, ti, nthr, num);
    for (i=0;i<num;i++)
    {
        if (i%nthr != ti)
        {
...
void process_filelist_mt(char** fnames, int num, int nthr)
{
    ...
    for (i=0;i<nthr;i++)
    {
        ...
        memcpy((char*)args[i]+sizeof(char**),(void*)(&i), sizeof(int));
        memcpy((char*)args[i]+sizeof(char**)+sizeof(int),(void*)(&num), sizeof(int));
        memcpy((char*)args[i]+sizeof(char**)+2*sizeof(int),(void*)(&nthr), sizeof(int));
...
{
    return 10*fabs((double)fi->r - (double)r) + fabs((double)fi->g - (double)g) + fabs((double)fi->b - (double)b);
}

double rgb_distance_nearest_pg(fileinfo* fi, unsigned char r, unsigned char g, unsigned char b)
{
    return fabs((double)fi->r - (double)r) + 10*fabs((double)fi->g - (double)g) + fabs((double)fi->b - (double)b);
}

double rgb_distance_nearest_pb(fileinfo* fi, unsigned char r, unsigned char g, unsigned char b)
{
    return fabs((double)fi->r - (double)r) + fabs((double)fi->g - (double)g) + 10*fabs((double)fi->b - (double)b);
}

double rgb_distance_nearest_prpg(fileinfo* fi, unsigned char r, unsigned char g, unsigned char b)
{
    return 10*fabs((double)fi->r - (double)r) + 10*fabs((double)fi->g - (double)g) + fabs((double)fi->b - (double)b);
}

double rgb_distance_nearest_pgpb(fileinfo* fi, unsigned char r, unsigned char g, unsigned char b)
{
    return fabs((double)fi->r - (double)r) + 10*fabs((double)fi->g - (double)g) + 10*fabs((double)fi->b - (double)b);
}

double rgb_distance_nearest_prpb(fileinfo* fi, unsigned char r, unsigned char g, unsigned char b)
{
    return 10*fabs((double)fi->r - (double)r) + fabs((double)fi->g - (double)g) + 10*fabs((double)fi->b - (double)b);
}

double rgb_distance_nearest_prpgpb(fileinfo* fi, unsigned char r, unsigned char g, unsigned char b)
{
    return ((rand() % 10) + 1) * fabs((double)fi->r - (double)r) + ((rand() % 10) + 1) * fabs((double)fi->g - (double)g) + ((rand() % 10) + 1) * fabs((double)fi->b - (double)b);
}
...
{
    return fabs((double)fi->g - (double)g);
}
...
int load_idxtable(fileinfo * fi)
{
    FILE* f;
    char* temp;
    int l, nr;
    unsigned int i, j;
    f = fopen(g_idx_filename, "r");
    if (!f)
    {
        printf("Cannot read from IDX file: %s\n", g_idx_filename);
        return 0;
    }
    temp = NULL;

    nr = fread((void*)&l, sizeof(int), 1, f);
    temp = (char*)malloc(l + 1);
    fread((void*)temp, l, 1, f);
    temp[l] = 0;
    if (strcmp(temp, g_dbin_file) || !nr)
    {
        printf("IDX file: g_dbin_file mismatch. File cannot be used.\n");
        printf("[\"%s\" != \"%s\"]\n", temp, g_dbin_file);
        fclose(f);
        free((void*)temp);
        return 0;
    ...
    fread((void*)temp, l, 1, f);
    temp[l] = 0;
    if (strcmp(temp, g_imgin_file) || !nr)
    {
        printf("IDX file: g_imgin_file mismatch. File cannot be used.\n");
        printf("[\"%s\" != \"%s\"]\n", temp, g_imgin_file);
        fclose(f);
        free((void*)temp);
        return 0;
    ...
    fread((void*)temp, l, 1, f);
    temp[l] = 0;
    if (strcmp(temp, g_snapdir) || !nr)
    {
        printf("IDX file: g_snapdir mismatch. File cannot be used.\n");
        printf("[\"%s\" != \"%s\"]\n", temp, g_snapdir);
        fclose(f);
        free((void*)temp);
        return 0;
    ...
    nr = fread((void*)&l, sizeof(unsigned int), 1, f);
    if (!nr || l != (int)g_img_x)
    {
        printf("IDX file: g_img_x mismatch. File cannot be used.\n");
        fclose(f);
        return 0;
    }
    ...
    if (!nr || l != (int)g_img_y)
    {
        printf("IDX file: g_img_y mismatch. File cannot be used.\n");
        fclose(f);
        return 0;
    }

    nr = fread((void*)&l, sizeof(unsigned int), 1, f);
    if (!nr || l != (int)g_img_sx)
    {
        printf("IDX file: g_img_sx mismatch. File cannot be used.\n");
        fclose(f);
        return 0;
    }

    nr = fread((void*)&l, sizeof(unsigned int), 1, f);
    if (!nr || l != (int)g_img_sy)
    {
        printf("IDX file: g_img_sy mismatch. File cannot be used.\n");
        fclose(f);
        return 0;
    }

    nr = fread((void*)&l, sizeof(long), 1, f);
    if (!nr || l != (int)g_part_x)
    {
        printf("IDX file: g_part_x mismatch. File cannot be used.\n");
        fclose(f);
        return 0;
    }

    nr = fread((void*)&l, sizeof(long), 1, f);
    if (!nr || l != (int)g_part_y)
    {
        printf("IDX file: g_part_y mismatch. File cannot be used.\n");
        fclose(f);
        return 0;
    }
    ...
    for (i=0;i<g_img_sx;i++)
    {
        ...
        {
            nr = fread((void*)(&g_img_idx[i][j]), sizeof(unsigned int), 1, f);
            if (!nr)
            {
                printf("IDX file: cannot read g_img_idx[%d][%d]. File cannot be used.\n", i, j);
                return 0;
            }
        }
    }
    ...
    if (!nr || l != (int)g_num)
    {
        printf("IDX file: g_num mismatch. File cannot be used.\n");
        fclose(f);
        return 0;
    }

    for (i=0;i<(unsigned int)g_num;i++)
    {
        nr = fread((void*)&(fi[i].n_used), sizeof(unsigned int), 1, f);
        if (!nr)
        {
            printf("IDX file: cannot read fi[%d].n_used. File cannot be used.\n", i);
            return 0;
        }
    }

    printf("Succesfully loaded IDX file.\n");
    fclose(f);
    return 1;
}

int save_idxtable(fileinfo* fi)
{
    FILE* f;
    int l, i, j;
    printf("Writing IDX file: %s\n", g_idx_filename);
    f = fopen(g_idx_filename, "w");
    if (!f)
    {
        printf("Cannot write to IDX file: %s\n", g_idx_filename);
        return 0;
    }

    l = strlen(g_dbin_file);
    fwrite((void*)&l, sizeof(int), 1, f);
    fwrite((void*)g_dbin_file, l, 1, f);

    l = strlen(g_imgin_file);
    fwrite((void*)&l, sizeof(int), 1, f);
    fwrite((void*)g_imgin_file, l, 1, f);

    l = strlen(g_snapdir);
    fwrite((void*)&l, sizeof(int), 1, f);
    fwrite((void*)g_snapdir, l, 1, f);

    fwrite((void*)&g_img_x, sizeof(unsigned int), 1, f);
    fwrite((void*)&g_img_y, sizeof(unsigned int), 1, f);
    fwrite((void*)&g_img_sx, sizeof(unsigned int), 1, f);
    fwrite((void*)&g_img_sy, sizeof(unsigned int), 1, f);
    fwrite((void*)&g_part_x, sizeof(long), 1, f);
    fwrite((void*)&g_part_y, sizeof(long), 1, f);

    for (i=0;i<(long)g_img_sx;i++)
    {
        for (j=0;j<(long)g_img_sy;j++)
        {
            fwrite((void*)(&g_img_idx[i][j]), sizeof(unsigned int), 1, f);
        }
    }

    fwrite((void*)&g_num, sizeof(long), 1, f);

    for (i=0;i<g_num;i++)
    {
        fwrite((void*)&(fi[i].n_used), sizeof(unsigned int), 1, f);
    }
...
void make_idx_filename()
{
    ...
    if (g_idx_filename)
    {
        printf("Using idx filename: %s\n", g_idx_filename);
        return;
    }

    fn = (char*)malloc(strlen(g_dbin_file) + 1);
    if (!fn) panic(HERE, "malloc fn");

    strcpy(fn, g_dbin_file);
    l = strlen(fn);
    for (i=l-1;i>=0;i--) if (fn[i] == '/' || fn[i] == '\\') break;
    if (i == 0) temp = fn;
    else temp = &fn[i + 1];

    /*printf("TEMP = %s, I = %d\n", temp, i);*/
    l = strlen(temp);
    for (i=l-1;i>=0;i--) if (temp[i] == '.') temp[i] = 0;

    /*printf("TEMP = %s\n", temp);*/
    g_idx_filename = (char*)malloc(strlen(g_imgin_file) + strlen(temp) + 20);
    sprintf(g_idx_filename, "%s-%s-%dx%d.idx", g_imgin_file, temp, g_img_sx, g_img_sy);
    free((void*)fn);
    printf("Using idx filename: %s\n", g_idx_filename);
}
...
int generate_indices(fileinfo * fi)
{
    ...
    if (load_idxtable(fi))
    {
        printf("Successfully loaded idx table, skipping generate idx.\n");
        return 1;
    }
...
void parse_options(int lb, char** par)
{
    ...
    if (get_option(lb, par, "idx-file", &optval) == 2 && optval)
    {
        g_idx_filename = (char*)malloc((sizeof(char) * strlen(optval)) + 1);
        if (!g_idx_filename) panic(HERE, "malloc g_idx_filename");
        strcpy(g_idx_filename, optval);
        free((void*)optval);
    }
