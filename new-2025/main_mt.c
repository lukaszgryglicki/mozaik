...
int process_file(fileinfo* fi, int num)
{
 ...
 char b, m;
 plik = NULL;
 if (!skip_bmpin) init_bmp(&bm_handle);
...
int save_dbfile(int num, int idx)
{
 FILE* out;
 int i, nproc;
 char * fn;

 fn = (char*)malloc( (strlen(g_dbout_file) + 2) * sizeof(char));
 sprintf(fn, "%s.%x", g_dbout_file, idx);
...
void process_filelist(char** fnames, int num)
{
 ...
 for (i=0;i<4;i++) p[i] = fork();
 for (i=0;i<num;i++) 
 {
	  if (skip(p, i)) continue;
	  if (!process_file(&g_file_info[i], num)) printf("Failed processing: %s\n", g_file_info[i].fname);
 }
 wait(NULL);
 save_dbfile(num, indx(p));
}
